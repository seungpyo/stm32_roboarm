...
void usart_init() {
	...
	*(GPIOA_MODER) &= ~GPIO_MODE_MASK(2, GPIO_MODE_RESET);
	*(GPIOA_MODER) |= GPIO_MODE_MASK(2, GPIO_MODE_AF);

	*(GPIOA_AFRL) |= GPIOA_AF_USART2_RX_MASK;
	*(GPIOA_MODER) &= ~GPIO_MODE_MASK(3, GPIO_MODE_RESET);
	*(GPIOA_MODER) |= GPIO_MODE_MASK(3, GPIO_MODE_AF);
